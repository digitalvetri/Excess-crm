...
# ── Boot-time secret self-test ───────────────────────────────────────────────────

# Cached result of the secret probe. None = not yet probed, True = secret accepted,
# False = secret rejected (401). Cached at module level so the network round-trip
# happens once; later calls re-log from the flag without re-POSTing.
_secret_ok: bool | None = None

# Loud, greppable banner so a misconfigured secret is impossible to miss in the logs.
_SECRET_FAIL_MSG = (
    "AGENT_SECRET_MISCONFIGURED — CRM rejected x-agent-secret with 401. "
    "AGENT_WEBHOOK_SECRET does not match the CRM; EVERY agent function call will fail. "
    "Fix the secret and redeploy."
)


async def _self_test_secret() -> None:
    """Probe the CRM agent-function endpoint once to verify AGENT_WEBHOOK_SECRET.

    The CRM checks the shared secret BEFORE validating the request body, so a 401 is an
    unambiguous "secret is wrong" signal. We use a side-effect-free action
    (``getActiveConfig``) with a ``playground-*`` callId so the CRM short-circuits the call
    lookup and never mutates anything. A 401 is logged as a loud ERROR and the result is
    cached; on later calls we re-log from the cached flag (no re-POST) so the error stays
    visible on every call without adding per-call latency.
    """
    global _secret_ok
    if _secret_ok is not None:
        if _secret_ok is False:
            logger.error(_SECRET_FAIL_MSG)
        return
    try:
        await crm_post(
            "getActiveConfig",
            "playground-boot-self-test",
            "boot-self-test",
            "boot-self-test",
        )
        _secret_ok = True
        logger.info("agent_secret_self_test ok — x-agent-secret accepted by CRM")
    except httpx.HTTPStatusError as exc:
        if exc.response.status_code == 401:
            _secret_ok = False
            logger.error(_SECRET_FAIL_MSG)
        else:
            # Any non-401 status means the secret itself was accepted (the body/lookup was
            # rejected downstream, which is expected for the dummy probe).
            _secret_ok = True
            logger.info(
                "agent_secret_self_test ok — secret accepted (probe status=%s)",
                exc.response.status_code,
            )
    except Exception as exc:  # noqa: BLE001 — network/DNS/timeout: can't conclude the secret is bad
        logger.warning("agent_secret_self_test inconclusive (network error) error=%s", exc)
...
class ExcessAgent(Agent):
    ...
    async def get_lead_info(self, run_ctx: RunContext) -> dict[str, Any]:
        ...
        except Exception as exc:  # noqa: BLE001 — never let a tool error become dead air
            logger.exception("get_lead_info failed lead=%s error=%s", self._lead_id, exc)
    ...
    async def get_product_info(self, run_ctx: RunContext, category: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("get_product_info failed lead=%s error=%s", self._lead_id, exc)
            return {"error": "temporarily_unavailable"}
    ...
    async def search_knowledge(self, run_ctx: RunContext, query: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("search_knowledge failed lead=%s error=%s", self._lead_id, exc)
            return {"error": "temporarily_unavailable"}
    ...
    async def get_follow_up_context(self, run_ctx: RunContext) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("get_follow_up_context failed lead=%s error=%s", self._lead_id, exc)
            return {"error": "temporarily_unavailable"}
    ...
    async def update_lead_stage(
        ...
        try:
            ...
            logger.exception("update_lead_stage failed lead=%s error=%s", self._lead_id, exc)
    ...
    async def update_conversion_status(self, run_ctx: RunContext, status: str) -> str:
        ...
        try:
            ...
            logger.exception("update_conversion_status failed lead=%s error=%s", self._lead_id, exc)
            return "temporarily_unavailable"
    ...
    async def schedule_follow_up(self, run_ctx: RunContext, scheduled_at: str) -> str:
        ...
        try:
            ...
            logger.exception("schedule_follow_up failed lead=%s error=%s", self._lead_id, exc)
            return "temporarily_unavailable"
    ...
    async def reschedule_follow_up(self, run_ctx: RunContext, scheduled_at: str) -> str:
        ...
        try:
            ...
            logger.exception("reschedule_follow_up failed lead=%s error=%s", self._lead_id, exc)
            return "temporarily_unavailable"
    ...
    async def schedule_appointment(
        ...
        try:
            ...
            logger.exception("schedule_appointment failed lead=%s error=%s", self._lead_id, exc)
            return {"error": "temporarily_unavailable"}
    ...
    async def mark_do_not_contact(self, run_ctx: RunContext) -> str:
        ...
        try:
            ...
            logger.exception("mark_do_not_contact failed lead=%s error=%s", self._lead_id, exc)
            return "temporarily_unavailable"
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    # Fail-loud secret self-test (probes once, then re-logs from the cached flag).
    await _self_test_secret()
    ...
    session = AgentSession(
        ...
        vad=vad,
        min_endpointing_delay=0.5,       # wait a touch longer so we don't cut the customer off mid-thought
    ...
    # Finalize the call (POST callEnded) at most once, and ONLY when the CUSTOMER hangs up —
    # not when an admin monitor stops watching. Identity conventions (see voice-dial.ts /
    # voice-agent.ts): customer = "sip-<phone>" (real call) or "playground-user-<id>" (browser
    # test); admin monitor = "monitor-<userId>". A monitor leaving must not end the call.
    call_finalized = False

    def _is_customer(participant: rtc.RemoteParticipant) -> bool:
        identity = participant.identity or ""
        if identity.startswith("monitor-"):
            return False
        return identity.startswith("sip-") or identity.startswith("playground-user-")

    @ctx.room.on("participant_disconnected")
    def _on_disconnect(participant: rtc.RemoteParticipant) -> None:
        nonlocal call_finalized
        if not _is_customer(participant):
            logger.info(
                "participant_disconnected ignored (not customer) identity=%s call_id=%s",
                participant.identity,
                call_id,
            )
            return
        if call_finalized:
            logger.info("callEnded already fired, ignoring duplicate disconnect call_id=%s", call_id)
            return
        call_finalized = True
