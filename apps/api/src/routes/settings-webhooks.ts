...
export const settingsWebhooksRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/', async (req, reply) => {
    ...
    const endpoints = await req.withTenant((tx) =>
  ...
  app.post('/', async (req, reply) => {
    ...
    const endpoint = await req.withTenant((tx) =>
  ...
  app.delete('/:id', async (req, reply) => {
    ...
    await req.withTenant((tx) =>
      tx.webhookEndpoint.deleteMany({ where: { id, tenantId: req.auth.tenantId } }),
    );
  ...
  app.post('/:id/rotate-secret', async (req, reply) => {
    ...
    const secret = randomBytes(32).toString('hex');
    await req.withTenant((tx) =>
      tx.webhookEndpoint.updateMany({ where: { id, tenantId: req.auth.tenantId }, data: { secret } }),
    );
