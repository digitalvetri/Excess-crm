...
```kotlin
// AES-256-GCM backed by hardware keystore
...
```kotlin
// Validate outbound URLs against allowlist
val allowedHosts = setOf("api.stripe.com", "api.github.com")

fun validateUrl(urlString: String): URL {
    val url = URL(urlString)
    require(url.protocol == "https") { "HTTPS required" }
    require(url.host in allowedHosts) { "Host not in allowlist" }
    val addr = InetAddress.getByName(url.host)
    require(!addr.isSiteLocalAddress && !addr.isLoopbackAddress) { "Private IP blocked" }
    return url
}
```
